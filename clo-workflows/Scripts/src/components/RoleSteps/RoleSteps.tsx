...
export class RoleSteps extends React.Component<any, any> {
    ...
    onStepButtonClick = (step: IStep): void => this.employeeStore.selectFocusStep(step)
        ...
        return (
            <div className="roleSteps-wrapper-styles">
                <div className="roleSteps-stepButtonRow-styles">
                    ...
                    this.sessionStore.currentUser.roles.filter(role => !!role.permittedSteps.length).map((role, index) => (
                        <div key={index}>
                            ...
                            role.permittedSteps.map((step: IStep, innerIndex: number) => {
                                ...
                                return (
                                    <CompoundButton
                                        ...
                                        secondaryText={`${pendingItemCount} Pending Item${pendingItemCount !== 1 ? "s" : ""}`}
                                        primary={!!pendingItemCount}
                                        text={step.name}
                                        checked={this.employeeStore.focusStep && step.name === this.employeeStore.focusStep.name}
                                    />
                                )
                            })}
                        </div>
