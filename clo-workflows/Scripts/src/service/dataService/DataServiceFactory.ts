    ...
    static getDataService(): IDataService {
        if (ENVIRONMENT === EnvType.LOCAL) {
            ...
            return new SpDataService(
                Utils.getQueryStringParameter("SPAppWebUrl"),
                Utils.getQueryStringParameter("SPHostUrl")
            )
             ...
             * is the local url (e.g. https://localhost:8080)
             * and the host url needs to be the host of the remote SP instanc
             */
