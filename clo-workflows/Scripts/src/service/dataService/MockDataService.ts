...
export class MockDataService implements IDataService {
    ...
    fetchEmployeeActiveProcesses(employee: IUser): Promise<Array<CloRequestElement>> {
        return Promise.resolve(this.utils.deepCopy(MockProcesses))
    }

    fetchRequestElementsById(ids: number[], listName: ListName): Promise<CloRequestElement[]> {
        switch (listName) {
            case ListName.PROJECTS:
                return Promise.resolve(this.utils.deepCopy(MockProjects.filter(project => ids.includes(project.Id as number))))
            case ListName.WORKS:
                return Promise.resolve(this.utils.deepCopy(MockWorks.filter(work => ids.includes(work.Id as number))))
            case ListName.PROJECTS:
                return Promise.resolve(this.utils.deepCopy(MockProcesses.filter(process => ids.includes(process.Id as number))))
            default:
    ...
    fetchClientActiveProjects(client: IUser): Promise<Array<CloRequestElement>> {
        return Promise.resolve(this.utils.deepCopy(MockProjects))
    }
    ...
    fetchNotes(source: NoteSource, scope: NoteScope, sourceId: string, attachedClientId: string): Promise<Array<INote>> {
        return source === NoteSource.PROJECT
            ? Promise.resolve(this.utils.deepCopy(MockNotes.filter(note => note.projectId === sourceId)))
            : Promise.resolve(this.utils.deepCopy(MockNotes.filter(note => note.workId === sourceId)))
    }
