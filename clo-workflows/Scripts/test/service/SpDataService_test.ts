...
ava.test(
    "clo data parser - ensure that the fieldSet contains every field from every table in addition to default fields",
    t => {
...
ava.test(
    "clo data parser - ensure that the parser correctly parses and filters a sample SharePoint response object",
    async t => {
        const cloParser = new CloRequestElementParser()
